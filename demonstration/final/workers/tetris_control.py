...
class tetris_control(bee.worker):
  ...
  b_rewards = buffer('pull', 'TetrisReward')
  
  
  get_rewards = triggerfunc(b_rewards)
  
  b_score = buffer('pull', 'int')
  
  
  get_score = triggerfunc(b_score)
  ...
  rewards = antenna('pull', 'TetrisReward')
