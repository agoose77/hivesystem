...
class staticbindbuilder(bindbuilder):
  def __new__(cls, name, bases, dic):
    ...
    morebindparameters = [(h[0], bindparameter(h[1])) for h in dicitems if h not in bindparameters and h[0] in oldbindparameters]
    ...
    def get_bindhiveworker(*args, **kwargs):
      class bindhiveworker(bindworker):
        ...
        @modifier
        def m_pause(self): self.v_running = False
        trigger(pause, m_pause)
        @modifier
        def m_resume(self): self.v_running = True
        ...
        def place(self):
          ...
          for b in binders: 
            inst = b.getinstance()
            if inst != None:
              params = inst.parametername, inst.parametervalue, str(inst.binderdroneinstance.__beename__), tuple(inst.antennanames)
