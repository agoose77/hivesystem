...
class emptyhivecontext(hivecontext_base):
  ...
  def __place__(self):
    ...
    for bee in self.bees: 
      ...
      posterior1 = isinstance(bee[1], beehelper) or isinstance(bee[1],io.antenna_io) or isinstance(bee[1],io.output_io)
    ...
    for beename,bee in self.bees:   
      try:   
        if hasattr(bee,"connect_contexts") and bee.connect_contexts != None and len(bee.connect_contexts) and bee.connect_contexts[-1] != "connect_contexts": 
...
class hivecontext(emptyhivecontext):
  def __place2__(self):
    ...
    if self.hivename == "m": ###
      ppp = []
      for beename, bee in self.bees:
        if isinstance(bee, connect) and not isinstance(bee.target, tuple) and isinstance(bee.source, tuple) and bee.source[1] == "evexc": 
          nam = bee.source[0]
...
class _hivebuilder(reg_beehelper):
  ...
  def __new__(metacls, name, bases, dic, specialmethods = [], **kargs):  
    ...
    globs = ["_has_exc", "_hive", "_import_parent_sockets", "_import_parent_plugins", "_import_parent_sockets_optional", "_import_parent_plugins_optional", "_import_parent_skip"]
    for g in globs:
      if g in dic:
        rdic[g] = dic[g]
      else:
    ...
    hivedict0 = {"_wrapped_hive":rhive, "guiparams":guiparams, "__metaclass__":metacls.__thisclass__, "_combobees":[], "_combobee_bases":combobee_bases_unique, "__hivecombined__":False}
