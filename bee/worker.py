...
class workerbuilder(reg_beehelper):
  ...
  def __new__(metacls, name, bases, dic, **kargs):
    forbidden =("__init__", "__place__", "_runtime_segment_classes", "_beename", "_runtime_segments", "__variabledict__")
    for f in forbidden:
    ...
    if emptyclass not in bases0:      
      if not ok: raise TypeError("Class definition of worker '%s': bee.worker with segments cannot be subclassed" % name)
    ...
    rworkerframe = type(nbc.__name__+":"+name, (nbc,), {"bee": rworker, "guiparams":guiparams, "__beedic__":dict(dic), "__helpers__":args})    
