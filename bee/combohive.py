...
class combohivewrapper(hivewrapper):
  ...
  def flatten(self, comboargs):
    ...
    def dictupdate(d):
      for k in d:
        ...
        v = d[k]
        if isinstance(v, list) or isinstance(v, tuple): ret[1][k] += v
        else: ret[1][k].append(v)
  ...
  def _combine_bee(self, bee):
    ...
    if isinstance(b, unregister): pass
    elif isinstance(b, hivecontext) and isinstance(name,int): pass
    else:
