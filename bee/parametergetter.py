...
class ParameterGetter(object):

    def __init__(self, name, access_stack=[], callstack=None):
        self.name = name

        if not isinstance(name, str):
            raise TypeError("bee.ParameterGetter must be called with a string argument")
    ...
    def __getattr__(self, attr):
        ...
        return ParameterGetter(self.name, access_stack, callstack)
