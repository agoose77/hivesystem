  ...
  try:   
    ...
    try:
      ...
      if exctype == WorkerError: 
        if python2: exec("raise WorkerError, value, tb")
        else:
    ...
    if self.inputmode == "push":
      ...
      if self.pull_input_socketclass == libcontext.socketclasses.socket_single_required:
        if len(self.connection_input) == 0:
          raise PullInputError("Segment '%s' has pull input and must have exactly one input" % (self.segmentname))
      if len(self.connection_input) > 0:
        libcontext.socket(("bee", "segment", "connection", get_ident(self.connection_input[0].identifier)), libcontext.socketclasses.socket_single_required(func))
    else:
      raise ValueError()
...
class runtime_weaver(object):
  ...
  def place(self):
    ...
    for connection in self.connection_output:    
      libcontext.plugin(("bee", "segment", "connection", get_ident(connection.identifier)), pluginclass(self.input))                    
...
class runtime_unweaver(object):
  ...
  def place(self):
    ...
    for onr, connection in enumerate(self._outputs):
      ...
      libcontext.socket(("bee", "segment", "connection", identifier), socketclass(func))                
    for connection in self.connection_input:            
      libcontext.plugin(("bee", "segment", "connection", get_ident(connection.identifier)), pluginclass(self.output))                    
...
class runtime_toggler(object):
  ...
  def place(self):
    ...
    for connection in self.connection_output:
      libcontext.socket(("bee", "segment", "connection", get_ident(connection.identifier)), socketclass(self.add_output))                    
...
class _runtime_antenna_push(object):
  ...
  def place(self):
    ...
    for connection in self._connection:
      libcontext.socket(("bee", "segment", "connection", get_ident(connection.identifier)), socketclass(self.add_connection))
...
class _runtime_output_pull(object):
  ...
  def place(self):
    ...
    libcontext.socket(("bee", "segment", "connection", get_ident(connection.identifier)), socketclass(self.set_input))
...
class _runtime_operator(object):
  ...
  def place(self):
    ...
    if self.intuple: r = self.runtuple
    else: r = self.runsingle
    ...
    for connection in self._connection_output:
      libcontext.socket(("bee", "segment", "connection", get_ident(connection.identifier)), socketclass(self.add_target))
