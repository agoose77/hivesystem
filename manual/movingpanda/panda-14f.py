...
class myscene(dragonfly.pandahive.spyderframe):
  ...
  a = Spyder.AxisSystem()
  a *= 0.005
  pandaclass = Spyder.ActorClass3D("models/panda-model", "egg", [("walk", "models/panda-walk4", "egg")], a,actorclassname="pandaclass")
  ...
  a = Spyder.AxisSystem()
  a *= 0.002
  pandaclass2 = Spyder.ActorClass3D("models/panda-model", "egg", [("walk", "models/panda-walk4", "egg")], a,actorclassname="pandaclass2")
...
class pandajumphive(bee.inithive):
  ...
  i = dragonfly.time.interval_time(time=0.7)
  connect(ksp, i.start)
      
  height = dragonfly.std.variable("float")(2.0)
  
  one = dragonfly.std.variable(("float","fraction"))(1.0)
  rest = dragonfly.op.pull.sub(("float","fraction"))()
  connect(one, rest.inp1)
  connect(i, rest.inp2)
  
  four = dragonfly.std.variable("float")(4.0)
  fourheight = dragonfly.op.pull.mul("float")()
  connect(four,fourheight.inp1)
  connect(height,fourheight.inp2)
  avgvelocity = dragonfly.op.pull.mul("float")()
  connect(fourheight, avgvelocity.inp1)
  connect(rest, avgvelocity.inp2)
  
  jump = dragonfly.op.pull.mul("float")()
  connect(avgvelocity, jump.inp1)
  connect(i, jump.inp2)
  ...
  connect(tick, t_jump)
  connect(ksp, tick.start)
  connect(i.reach_end, tick.stop)

class pandajumphive_forward(pandajumphive):  
  v_forward = dragonfly.std.variable("float")(-0.1)
  t_forward = dragonfly.std.transistor("float")()
  tr_forward = dragonfly.scene.bound.translateY()
  connect(v_forward, t_forward)
  connect(t_forward, tr_forward)
  connect("tick", t_forward)
...
class myhive(dragonfly.pandahive.pandahive):
  ...
  c_hivereg.register_hive("pandajump", pandajumphive_forward)
