  ...
  for key,value in bge.events.__dict__.items():
    if not key.endswith("KEY"): 
      if key.startswith("PAD"): 
        if len(key) == 4: trans[value] = key[-1]
        elif key == "PADPERIOD": trans[value] = "."
        elif key == "PADMINUS": trans[value] = "-"
        elif key == "PADENTER": trans[value] = "RETURN"      
      else: continue
    key = key[:-3]
    if len(key) == 1:
      ...
      trans[value] = key
    elif key == "ZERO": trans[value] = "0"
    elif key == "ONE": trans[value] = "1"
    elif key == "TWO": trans[value] = "2"
    elif key == "THREE": trans[value] = "3"
    elif key == "FOUR": trans[value] = "4"
    elif key == "FIVE": trans[value] = "5"
    elif key == "SIX": trans[value] = "6"
    elif key == "SEVEN": trans[value] = "7"
    elif key == "EIGHT": trans[value] = "8"
    elif key == "NINE": trans[value] = "9"
    elif key.startswith("PAD"): 
      if key == "PADSLASH": trans[value] = "/"
      elif key == "PADASTER": trans[value] = "*"
      elif key == "PADPLUS": trans[value] = "+"
    elif key == "ACCENTGRAVE": trans[value] = "`"
    elif key == "BACKSLASH": trans[value] = "\\"
    elif key == "COMMA": trans[value] = ","
    elif key == "DEL": trans[value] = "DELETE"
    elif key == "END": trans[value] = "DELETE"
    elif key == "EQUAL": trans[value] = "="
    elif key == "ESC": trans[value] = "ESCAPE"
    elif key == "HOME": trans[value] = "HOME"
    elif key == "INSERT": trans[value] = "INSERT"
    elif key == "LEFTBRACKET": trans[value] = "["
    #LINEFEEDKEY? 
    elif key == "RIGHTBRACKET": trans[value] = "]"
    elif key == "MINUS": trans[value] = "-"
    elif key == "PAGEDOWN": trans[value] = "PAGEDOWN"
    elif key == "PAGEUP": trans[value] = "PAGEUP"
    elif key == "PAUSE": trans[value] = "PAUSE"
    elif key == "PERIOD": trans[value] = "."
    elif key == "QUOTE": trans[value] = "'"
    elif key == "ENTER": trans[value] = "RETURN"
    elif key == "SEMICOLON": trans[value] = ";"
    elif key == "SLASH": trans[value] = "/"
    elif key == "SPACE": trans[value] = "SPACE"
    elif key == "TAB": trans[value] = "TAB"
    
          
    #TODO: modifiers?
    elif key.endswith("ARROW"): trans[value] = key[:-len("ARROW")]
