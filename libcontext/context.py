...
def find_subcontexts(context):
    ...
    subcontextnames = [k for k in contexts.keys() if
                       len(k) > len(currcontextname) and k[:len(currcontextname)] == currcontextname]
...
def decode_context(contextargs):
    ...
    for s in ret:
        if not hasattr(s, "decode") and not hasattr(s, "encode"): raise TypeError(
            "Invalid context %s" % str(contextargs))
    return ret
...
class context(object):
    ...
    def close(self):
        ...
        subcontextnames0 = [k for k in contexts.keys() if
                            len(k) > len(selfcontextname) and k[:len(selfcontextname)] == selfcontextname]
        ...
        for subcontextname in subcontextnames0:
            ...
            for s in subcontextnames0:
                ...
                if s[:-1] == subcontextname:
                    subnames0.append(s)
                elif s[-1] == "evexc" and s[:-2] == subcontextname:
                    subnames0.append(s)
