...
class WorkerBuilder(object):
  ...
  def build_metaworker(self, id_, metaworker):
    ...
    if memberorder is not None:
      p = zip(paramnames, paramtypelist)
      p.sort(key=partial(keyfunc, memberorder))
      paramnames, paramtypelist = [pp[0] for pp in p], [pp[1] for pp in p]
