    ...
    for bee_id, bee in bees.items():
      ...
      if workertype.startswith("bees.io"):
        if workertype in ("bees.io.push_antenna", "bees.io.pull_antenna"):
          ...
          if len(wcon_out) == 0:
            print("Warning: %s '%s' does not have any outgoing connections, bee is not saved!" % (workertype, bee_id))
            continue
          elif len(wcon_out) > 1:
            print("Warning: %s '%s' has multiple outgoing connections, selecting the first..." % (workertype, bee_id))
          mio = "antenna"
          hook = "inhook"
          ...
          if len(wcon_in) == 0:
            print("Warning: %s '%s' does not have any incoming connections, bee is not saved!" % (workertype, bee_id))
            continue
          elif len(wcon_in) > 1:
            print("Warning: %s '%s' has multiple incoming connections, selecting the first..." % (workertype, bee_id))
          mio = "output"
          hook = "outhook"
          targetcon = wcon_in[0]          
          mapattr = "_outmapr"          
