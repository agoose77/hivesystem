...
def get_formtype(form, name):
  ...
  if issubclass(typ, Spyder.File):
    ...
    if not hasattr(form, "file"):
      raise TypeError("Cannot generate Blender layout for \"%s\"(%s): a file type (file attribute) must be specified" % (name, typnam))
    if hasattr(form, "options"):
      raise TypeError("Cannot generate Blender layout for \"%s\"(%s): file elements cannot have options" % (name, typnam))
    formtype = "file"
...
class blenderview:
  ...
  def set(self, v):
    ...
    if isinstance(v, self.type): vv = v
    else: vv = self.type(v)
    for prop in self._properties:
      if self._form.arraycount > 0:
        try:
